...
##-------------Evaluate inference with Missforest (Random forest method)-------------

fct_missforest_evaluation <- function(data_to_infer, 
                                      model_iteration = 100,
  ...
  model_eval_missforest <- pbmcapply::pbmclapply(c(1:model_iteration), 
      ...
      phylo <- data_to_infer |> dplyr::select(phylum, class, order)
      traits <- data_to_infer |> dplyr::select(-phylum, -class, -order)
  
      # data_withNA <- traits |> 
      #   missForest::prodNA(noNA = prop_NA) |> 
      #   dplyr::bind_cols(phylo)
  ...
  # save(model_eval_missforest, file = here::here("outputs", "traits_inferred_MissForest.Rdata"))
  # # load( file = here::here("outputs", "traits_inferred_MissForest.Rdata"))

} ## END OF fct_missforest_evaluation
...
## ----------------------- Test with families in Misssforest -----------------
fct_missforest_evaluation_with_family <- function(data_to_infer){ 
                                      
  model_eval_missforest_with_family <- pbmcapply::pbmclapply(c(1:50), 
                                                 mc.cores = parallel::detectCores() - 3, 
                                                 FUN = function(N){
    data_to_infer <- traits_data 
                                                   
    traits <- data_to_infer |> dplyr::select(-phylum, -class, -order, -family)
    ...
    ## Imputing data ##
    impute = missForest::missForest(data_withNA, 
                                   maxiter = 10, ntree = 150, ######################## maxiter = 10, ntree = 100 when it's OK
                                   variablewise = T, verbose = T)
    ...
    eval_factors <- traits_data_factors |> 
     dplyr::left_join(data_withNA_factors) |> 
     dplyr::left_join(imputed_factors) |> 
     dplyr::select(-all_of(family_list)) |> 
     dplyr::mutate(missForest = N) |> #identify the iteration N of the mclapply
     dplyr::filter(is.na(obs_NA) & !is.na(observed)) # keep only rows where we can assess the imputation accuracy
  ...
  ### Save complete results ###
  save(model_eval_missforest_with_family, file = here::here("outputs", "traits_inferred_MissForest_with_fam.Rdata"))
  # load( file = here::here("outputs", "traits_inferred_MissForest.Rdata"))

} ## END of fct_missforest_evaluation_with_family


## ----------------------- Test NA proportion on prediction performance -----------------

fct_test_NA_proportion <- function(data_to_infer = data_to_infer){
  
  res <- pbmcapply::pbmclapply(seq(0.1,0.9,0.1), mc.cores = 2, FUN = function(prop_NA){
    
    res_MF <- fct_missforest_evaluation(data_to_infer,
                                        model_iteration = 10,
                                        maxiter_mf=5,
                                        ntree_mf=10, 
                                        prop_NA = prop_NA)
    
    flat_list <- unlist(res_MF, recursive = F)
    ...
    ##overall result
    estimates_factors <- raw_estimates_factor |> 
      dplyr::group_by(missForest, prop_NA) |> 
      dplyr::summarise(precision = mean(observed==imputed)) |> 
      dplyr::mutate(type = "accuracy")
    
    estimates_num <- raw_estimates_num |> 
      ...
      dplyr::ungroup() |> 
      dplyr::group_by(missForest, prop_NA) |> 
      dplyr::summarise(precision = cor(observed, imputed)) |> 
      dplyr::mutate(type = "r-pearson")

    estimate_res <- rbind(estimates_factors, estimates_num)
    
    ## result per traits
    estimates_factors <- raw_estimates_factor |> 
      dplyr::group_by(missForest, prop_NA, variable) |> 
      dplyr::summarise(precision = mean(observed==imputed)) |> 
      dplyr::mutate(type = "accuracy")
    ...
    estimate_res_per_traits <- rbind(estimates_factors, estimates_num)
    
    list(estimate_res, estimate_res_per_traits)
  }) # End of mclapply
  
  flat_res <- unlist(res, recursive = F)
  
  estimates_all <- do.call(rbind,flat_res[seq(1,length(flat_res),2)])
  estimates_per_traits <- do.call(rbind,flat_res[seq(2,length(flat_res),2)])

  list(estimates_all, estimates_per_traits)
} #END OF FUNCTIUN fct_test_NA_proportion
