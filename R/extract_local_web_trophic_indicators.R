...
#' Principal function to extract trophic web indicators from the local webs
#'
#' @param P_A_data presence absence matrix
#' @param biomass_data relative biomass of species in each survey
#' @param Lniche Metaweb
#' @param mc.cores number of core for parallel computing
#' @param bin_threshold threshold of probability of predation
#'
#' @return
#' @export
#'
...
trophic_indicators_survey <- as.data.frame(cbind(survey_id = rownames(PA_matrix_survey), 
                                                 trophic_indicators_survey)) |>
  dplyr::mutate(across(.cols = everything(), .fns = as.numeric, .names = "{.col}" ))

save(trophic_indicators_survey, 
     file= here::here("outputs", "2e_trophic_indicators_survey.Rdata"))
