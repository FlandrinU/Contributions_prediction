...
#' @param original_name name of the column in which there is the species name
#' @param mc_cores number of cores used for the Parallelization. Put 1 if problems with rfishbase
#' 
#'
#' @return
#' @export
...
code_sp_check <- function(data,
                          original_name = "rls_species_name",
                          mc_cores = 10) {
  
  colnames(data)[colnames(data) == original_name] <- "species_name"
  var <- unique(data$species_name)
  
  check <- do.call(rbind, pbmcapply::pbmclapply(1:length(var), function(i) {
    if (i %% 500 == 0) {
      Sys.sleep(sample(1:60, 1))
    } # Avoid problem with API request
    ...
    if (sum(is.na(check_ind[1, ])) == 9) {
      ...
      ) # check if very not a problem in the filter accepted, synonym...should be 1
    } # end of else
  }, mc.cores = mc_cores)) # end of lapply
  
  res <- merge(data, check, by = "species_name")
  colnames(res)[colnames(res) == "species_name"] <- original_name
