...
###--------------------- Simulation loop ---------------------
accuracy_model <- pbmcapply::pbmclapply (1:999,mc.cores=parallel::detectCores()-5, function(i) { 
  ...
  estim.pars = GenSA::GenSA(par = pars80, fn = model, lower = par_lo, upper= par_hi, 
                            control = list(verbose =TRUE, max.time = 50, smooth=FALSE),
                            data = data80) #Search for parameters maximizing the posteriori probability of these observed interactions
  ...
  pLM <- pLMFitted(MPrey20,MPred20,Pars) # function imported from Model_GenSA.R code
  ...
  CMX1 <- PresenceAbsence::cmx(data_valid, na.rm=TRUE, threshold=0.5)
  boyce_index <- boyce(simuldata,obsvdata,PEplot=F)
  data.frame(Sens=PresenceAbsence::sensitivity(CMX1, st.dev=FALSE) *100,Boyce=boyce_index)
...
save(file= here::here("outputs", "2e_accuracy_metaweb_model_boyce_index.Rdata"),accuracy_model)

summary(accuracy_model$Boyce)
# Min. 1st Qu.  Median    Mean 3rd Qu.    Max. 
# 0.4390  0.5360  0.6170  0.5909  0.6380  0.7100 

###---------------------  Model evaluation ---------------------
#### Test on some species ###
pars <- read.table(here::here("outputs", "2e_niche_model_parameters.txt") )
data = final_barnes_data
pred <- c("Gadus morhua", "Zeus faber","Trachurus trachurus", "Thunnus thynnus")
name_file = "Allometric_niche_model_"

for (i in pred){
  jpeg(here::here("figures", "food_web", paste0(name_file,i,".jpg")), width = 20, 
       height = 10, units = "cm", pointsize = 12, res = 400)
