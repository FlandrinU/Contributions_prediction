          ...
          geo.weighted = TRUE, print.results = TRUE) 
{
  ...
  Gl.Model <- eval(substitute(ranger(formula, data = dframe, 
  ...
  LM_LEst <- as.data.frame(setNames(replicate(ModelVarNo, 
  ...
  LM_GofFit <- data.frame(y = numeric(0), LM_yfitOOB = numeric(0), 
                          ...
                          LM_MSE = numeric(0), LM_Rsq100 = numeric(0), LPerm = numeric(0))
  for (m in 1:Obs) {
    DNeighbour <- Dij[, m]
